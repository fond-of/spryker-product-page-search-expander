<?php

namespace FondOfSpryker\Shared\ProductPageSearchExpander;

interface ProductPageSearchExpanderConstants
{
    public const PRODUCT_ATTRIBUTES = 'attributes';

    public const MODEL_KEY = 'model_key';

    public const IS_SOLD_OUT = 'is_sold_out';

    public const MODEL_SHORT = 'model_short';

    public const SIZE = 'size';

    public const STYLE_UNIQUE = 'style_unique';

    public const STYLE_KEY = 'style_key';

    public const AVAILABLE = 'available';

    public const IMAGE_SETS = 'image_sets';

    public const OPTION_DONT_MERGE_SIZES = 'DONT_MERGE_SIZES';

    public const OPTION_SIZE_SWITCHER = 'PLUGIN_SIZE_SWITCHER';

    public const PLUGIN_PRODUCT_AVAILABLE = 'PLUGIN_PRODUCT_AVAILABLE';

    public const PLUGIN_PRODUCT_IS_SOLD_OUT = 'PLUGIN_PRODUCT_IS_SOLD_OUT';

    public const PLUGIN_PRODUCT_MODEL_KEY = 'PLUGIN_PRODUCT_MODEL_KEY';

    public const PLUGIN_PRODUCT_MODEL_SHORT = 'PLUGIN_PRODUCT_MODEL_SHORT';

    public const PLUGIN_PRODUCT_SIZE = 'PLUGIN_PRODUCT_SIZE';

    public const PLUGIN_PRODUCT_STYLE_KEY = 'PLUGIN_PRODUCT_STYLE_KEY';

    public const PLUGIN_PRODUCT_IMAGE_SETS = 'PLUGIN_PRODUCT_IMAGE_SETS';

    public const PLUGIN_PRODUCT_MODEL_PRODUCT_DATA = 'PLUGIN_PRODUCT_MODEL_PRODUCT_DATA';
}
